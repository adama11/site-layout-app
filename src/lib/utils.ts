...
export function setNumberOfTransformersNeeded(
  ...
  const didAddTransformer =
    numberOfTransformersNeeded > existingTransformerCount;
  return [siteDevices, didAddTransformer];
}

export function computeTotalArea(siteDevices: SiteDevices): number {
  const area = Object.entries(siteDevices).reduce(
    (acc: number, [deviceName, deviceCount]: [string, number]) => {
      return (
        acc +
        deviceData[deviceName].xDimension *
          deviceData[deviceName].yDimension *
          deviceCount
      );
    },
    0
...
export function computeTotalCost(siteDevices: SiteDevices): number {
  const cost = Object.entries(siteDevices).reduce(
    (acc: number, [deviceName, deviceCount]: [string, number]) => {
...
export function computeTotalPower(siteDevices: SiteDevices): number {
  const power = Object.entries(siteDevices).reduce(
    (acc: number, [deviceName, deviceCount]: [string, number]) => {
...
export const dateFromLastUpdatedText = (lastUpdated: string | undefined) => {
  if (lastUpdated === undefined) {
    return undefined;
  }
  return new Date(Date.parse(`${lastUpdated} UTC`));
};
