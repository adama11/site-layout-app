...
import { TbChevronDown, TbCloudUpload, TbLoader2, TbRefresh } from 'react-icons/tb';
...
export const Header = ({
  ...
  const lastUpdateText = lastUpdated ? lastUpdated.toLocaleString() : 'Not saved';
  return (
    <header className="bg-slate-900 flex w-full flex-wrap items-center justify-between px-3 shadow-md">
      ...
      <div className="flex grow basis-0 justify-end space-x-4">
        ...
        <Menubar>
          <MenubarMenu>
            <MenubarTrigger>
              <div className="mr-2">{sessionName ? sessionName : <TbLoader2 className="animate-spin" />}</div>
              <TbChevronDown />
            </MenubarTrigger>
            <MenubarContent>
              ...
              <MenubarSeparator />
              <div className="px-2 pt-2 text-xs font-bold uppercase text-slate-400">Last saved</div>
